...
export const OP_HELP: Record<string, OpHelp> = {
  ...
  mulFields: { detail: "The element's algebra product: ordinary × for scalar, complex multiply for complex, Hamilton product for quaternion.", math: "z\\,w = (ac - bd) + (ad + bc)\\,i" },
  ...
  fdwt: { detail: "Separable 2D wavelet transform → a packed Mallat coefficient pyramid (the editable wavelet-domain representation).", math: "x \\;\\xrightarrow{\\;W\\;}\\; \\{\\,c_{LL},\\,c_{LH},\\,c_{HL},\\,c_{HH}\\,\\}" },
  idwt: { detail: "Inverse wavelet transform: resynthesise the spatial field from coefficients.", math: "c \\;\\xrightarrow{\\;W^{-1}\\;}\\; x" },
  thresholdDetail: { detail: "Wavelet shrinkage: pull detail coefficients toward zero (denoise / compress), leaving the LL approximation.", math: "\\eta_t(x) = \\operatorname{sgn}(x)\\,\\max(|x| - t,\\, 0)" },
