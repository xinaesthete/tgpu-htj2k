...
import { applyAffine, chunkArrayBox, chunkKey, TileCache, type Loader, type Multiscale, type Selection, type Tile, type Vec3 } from "../../../src/datasource";
...
export class TileRenderer {
  ...
  constructor(private ms: Multiscale, private loader: Loader, maxBytes = 256 * 1024 * 1024) {
  ...
  update(selection: Selection): void {
    ...
    for (const sc of selection.chunks) {
      ...
      if (tex) { this.addMesh(k, sc.id, tex); continue; }
  ...
  private addMesh(k: string, id: Tile["id"], tex: THREE.Texture): void {
    ...
    const c00 = p(lo[0], lo[1]), c10 = p(hi[0], lo[1]), c11 = p(hi[0], hi[1]), c01 = p(lo[0], hi[1]);
