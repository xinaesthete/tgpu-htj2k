...
This doc is the **wavelet / image-signal front**: data as a regular grid of signal
samples. A sibling front handles **discrete-cell** spatial analysis (points and
shapes — cell centroids, transcripts, boundaries) in
[`gpu-spatial-analysis-toolbox.md`](gpu-spatial-analysis-toolbox.md). The two share
this runtime and the kernel runner; the bridge between them is rasterising point
clouds into the grids these primitives consume (density splat, KDE → convolution).
...
See also [`gpu-spatial-analysis-toolbox.md`](gpu-spatial-analysis-toolbox.md) (the
discrete-cell sibling front), [`performance-report.md`](performance-report.md), and
