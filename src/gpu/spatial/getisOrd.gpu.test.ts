...
describe("getisOrdGpu", () => {
  ...
  it("flags a dense region as a hot spot (high positive z)", async () => {
    ...
    for (let row = 12; row < 18; row++) for (let col = 12; col < 18; col++) grid[row * w + col] = 1.0;
    const { z } = await getisOrdGpu(grid, w, h, { radius });
  ...
  it("composes splat -> hotspots end to end (pointHotspotsGpu)", async () => {
    ...
    let best = -Infinity,
      bi = -1;
    for (let i = 0; i < field.z.length; i++)
      if (field.z[i]! > best) {
        best = field.z[i]!;
        bi = i;
      }
    const col = bi % field.width,
      row = Math.floor(bi / field.width);
