...
function adaptiveCpu(
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < n; j++) {
      ...
      const a = Math.exp(-d2 * invI), b = Math.exp(-d2 * invJ);
      out[i * n + j] = a + b - a * b;
    }
  }
  return out;
...
describe("fuzzyAdjacencyAdaptive (UMAP-style)", () => {
  it("matches the t-conorm CPU golden for given per-point bandwidths", async () => {
    const n = 12;
    const xs: number[] = [], ys: number[] = [], rho: number[] = [];
    for (let i = 0; i < n; i++) {
