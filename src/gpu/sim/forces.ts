...
export function solenoidForce(pos: Vec3, strength: number, coilRadius = 2.5): Vec3 {
  const x = pos[0], y = pos[1], z = pos[2];
  const lxz = Math.hypot(x, z);
  const coeff = 0.045 * strength;
  if (lxz < EPS) return [0, -coeff, 0];
  const R = coilRadius;
...
export function partnerOrbitForce(pos: Vec3, vel: Vec3, partner: Vec3, strength: number): Vec3 {
