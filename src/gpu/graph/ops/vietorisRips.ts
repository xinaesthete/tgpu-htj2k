...
export const vietorisRipsOp: OpType = {
  ...
  params: [
    { name: "maxScale", type: "number", default: 0, min: 0, max: 100, step: 0.01, describe: "cap simplex filtration (0 = auto)" },
  ],
  inferShapes() {
  ...
  async execute(_ctx, inputs, params) {
    ...
    const result: PersistenceResult = vietorisRipsPersistence(
      inputs[0]!.data!,
      n,
      maxScale > 0 ? { maxScale } : {},
    );
